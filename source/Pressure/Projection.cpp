            ...
            if ((Fire::getCellType(_phi->grid->valueAtIndex(i, j, k)) == IGNITED
                ...
                if (i+1 == 0 || j == 0 || i+1 == (_phi->grid->xdim()-1) || j == (_phi->grid->ydim()-1)) {
                    ...
                    if (Fire::getCellType(_phi->grid->valueAtIndex(i+1, j, k)) == IGNITED) {
                        ...
                        A->add_to_element(row, row, scale/FirePresets::rhoh);
                    }
    ...
    for (GridFieldIterator<double> it = _phi->grid->iterator(); !it.done(); it.next()) {
        ...
        if (k == 0) {
            ...
            if (Fire::getCellType(_phi->grid->valueAtIndex(i, j, k)) == BLUECORE
                ...
                double density = 0;
                if (currType == IGNITED) {
                    density = FirePresets::rhof;
                }else if (currType == BLUECORE){
                    density = FirePresets::rhoh;
                }
...
double Projection::getAlpha(int i, int j, int k, DirectionEnums d) const{
	...
	if(Fire::getCellType(thisCell) == IGNITED && Fire::getCellType(otherCell) == IGNITED)
		return 1;
	else if(Fire::getCellType(thisCell) == IGNITED && Fire::getCellType(otherCell) == BLUECORE)
		return (thisCell /(thisCell - otherCell));
	else if(Fire::getCellType(thisCell) == BLUECORE && Fire::getCellType(otherCell) == IGNITED)
		return 1.0 - (thisCell /(thisCell - otherCell));
...
double Projection::getDensity(int i, int j, int k, DirectionEnums d) const{
    ...
    return FirePresets::rhof*alfa+(1.0-alfa)*FirePresets::rhoh;
}
